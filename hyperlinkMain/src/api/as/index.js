...
export const getStoreAsList = async (page = 0, size = 15, keyWord = '', status = 'all') => {
    try {
        const response = await api.get(`/api/as/list?page=${page}&size=${size}&keyWord=${keyWord}&status=${status}`);
        return { success: true, data: response.data.data };
    } catch (error) {
        const errorData = error.response?.data || { message: '네트워크 오류 또는 서버 응답 없음' };
        errorData.success = false;
...
export const getHeadOfficeAsList = async (page = 0, size = 15, keyWord = '', status = 'all') => {
    try {
        const response = await api.get(`/api/as/list/paging?page=${page}&size=${size}&keyWord=${keyWord}&status=${status}`);
        return { success: true, data: response.data.data };
    } catch (error) {
        const errorData = error.response?.data || { message: '네트워크 오류 또는 서버 응답 없음' };
        errorData.success = false;
...
export const getASStatus = async () => {
    try {
        const response = await api.get(`/api/as/read/status`);
        return { success: true, data: response.data.data };
    } catch (error) {
        const errorData = error.response?.data || { message: '네트워크 오류 또는 서버 응답 없음' };
        errorData.success = false;
