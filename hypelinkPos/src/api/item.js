...
// 특정 매장의 전체 상품 조회 (페이징)
const getItemList = async (storeId, page = 0, size = 50) => {
    const url = "/api/store/item/list"
    let result = { success: false }

    await api.get(url, { params: { storeId, page, size } })
        .then((response) => {
...
// 상품 검색 (한글명, 영문명, 바코드)
const searchItems = async (storeId, keyword, page = 0, size = 50) => {
    const url = "/api/store/item/search"
    let result = { success: false }

    await api.get(url, { params: { storeId, keyword, page, size } })
        .then((response) => {
            result = { success: true, data: response.data.data }
        })
        .catch((error) => {
...
// 카테고리별 상품 조회
const getItemsByCategory = async (storeId, category, page = 0, size = 50) => {
    const url = "/api/store/item/category"
    let result = { success: false }

    await api.get(url, { params: { storeId, category, page, size } })
        .then((response) => {
            result = { success: true, data: response.data.data }
        })
        .catch((error) => {
...
// 재고 부족 상품 조회
const getLowStockItems = async (storeId, minStock = 10, page = 0, size = 50) => {
    const url = "/api/store/item/low-stock"
    let result = { success: false }

    await api.get(url, { params: { storeId, minStock, page, size } })
        .then((response) => {
            result = { success: true, data: response.data.data }
        })
        .catch((error) => {
